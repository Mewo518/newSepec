#这个是new。py的优化，new。py中的数据集用的是自己创建的随机的
# 本文件用的是下载下来的数据集
import torch
import time
import numpy as np
...
class ExperimentSystem:
    ...
    def __init__(self):
        ...
        self.dataset = self.dataset.filter(
            lambda x: len(x["text"].strip()) > 100 and not x["text"].startswith(" =")
        )
        self.dataset = self.dataset.select(range(100))  # 使用前100条样本
    ...
    def _prepare_samples(self):
        ...
        for text in self.dataset["text"]:
            ...
            processed.append(first_sentence[:500])  # 限制最大长度
        return processed
...
if __name__ == "__main__":
    # 初始化实验系统（自动加载WikiText-103）
    exp_system = ExperimentSystem()

    # 运行实验（控制最大样本数）
    try:
        exp_system.run_experiments(num_samples=50)  # 使用50个样本
    except torch.cuda.OutOfMemoryError:
        print("CUDA OOM Error! Trying with CPU...")
        exp_system = ExperimentSystem()  # 重新初始化到CPU
        exp_system.run_experiments(num_samples=10)  # 减少样本量
